...
from AarohixMusic.utils import get_readable_time
...
from AarohixMusic.utils.extraction import extract_user
from config import BANNED_USERS


@nand.on_message(filters.command(["gban", "globalban"]) & SUDOERS)
@language
...
@nand.on_message(filters.command(["ungban"]) & SUDOERS)
@language
async def global_un(client, message: Message, _):
    ...
    for chat_id in served_chats:
        try:
            await nand.unban_chat_member(chat_id, user.id)
            number_of_chats += 1
        except FloodWait as fw:
            await asyncio.sleep(int(fw.value))
        except:
...
@nand.on_message(filters.command(["gbannedusers", "gbanlist"]) & SUDOERS)
@language
