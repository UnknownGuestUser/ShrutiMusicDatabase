...
from AarohixMusic.utils.database import blacklist_chat, blacklisted_chats, whitelist_chat
from AarohixMusic.utils.decorators.language import language
from config import BANNED_USERS


@nand.on_message(filters.command(["blchat", "blacklistchat"]) & SUDOERS)
@language
...
@nand.on_message(filters.command(["blchats", "blacklistedchats"]) & ~BANNED_USERS)
@language
async def all_chats(client, message: Message, _):
    ...
    for count, chat_id in enumerate(await blacklisted_chats(), 1):
        try:
            title = (await nand.get_chat(chat_id)).title
        except:
            title = "ᴘʀɪᴠᴀᴛᴇ ᴄʜᴀᴛ"
        j = 1
        text += f"{count}. {title}[<code>{chat_id}</code>]\n"
    if j == 0:
        await message.reply_text(_["black_8"].format(nand.mention))
    else:
