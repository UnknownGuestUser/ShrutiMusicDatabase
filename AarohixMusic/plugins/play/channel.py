...
from AarohixMusic.utils.database import set_cmode
from AarohixMusic.utils.decorators.admins import AdminActual
from config import BANNED_USERS


@nand.on_message(filters.command(["channelplay"]) & filters.group & ~BANNED_USERS)
@AdminActual
async def playmode_(client, message: Message, _):
    ...
    if (str(query)).lower() == "disable":
        ...
        chat = await nand.get_chat(message.chat.id)
        if chat.linked_chat:
        ...
        try:
            chat = await nand.get_chat(query)
        except:
