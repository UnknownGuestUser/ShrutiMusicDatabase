...
from AarohixMusic.misc import db
from AarohixMusic.utils.database import get_assistant, get_authuser_names, get_cmode
from AarohixMusic.utils.decorators import ActualAdminCB, AdminActual, language
from AarohixMusic.utils.formatters import alpha_to_int, get_readable_time
...
@nand.on_message(filters.command(["reboot"]) & filters.group & ~BANNED_USERS)
@AdminActual
async def restartbot(client, message: Message, _):
    mystic = await message.reply_text(_["reload_4"].format(nand.mention))
    ...
    if chat_id:
        try:
            got = await nand.get_chat(chat_id)
        except:
            pass
        ...
        try:
            ...
            pass
    return await mystic.edit_text(_["reload_5"].format(nand.mention))
...
@nand.on_callback_query(filters.regex("stop_downloading") & ~BANNED_USERS)
@ActualAdminCB
