...
import config
from AarohixMusic import LOGGER, nand, userbot
from AarohixMusic.core.call import Shruti
from AarohixMusic.misc import sudo
from AarohixMusic.plugins import ALL_MODULES
from AarohixMusic.utils.database import get_banned_users, get_gbanned
...
async def init():
    ...
    for all_module in ALL_MODULES:
        importlib.import_module("AarohixMusic.plugins" + all_module)
    LOGGER("AarohixMusic.plugins").info("Successfully Imported Modules...")
    await userbot.start()
    await Shruti.start()
    try:
        await Shruti.stream_call("https://te.legra.ph/file/29f784eb49d230ab62e9e.mp4")
    except NoActiveGroupCall:
        LOGGER("AarohixMusic").error(
    ...
    await nand.stop()
    await userbot.stop()
    LOGGER("AarohixMusic").info("Stopping AarohixMusic Music Bot...")
